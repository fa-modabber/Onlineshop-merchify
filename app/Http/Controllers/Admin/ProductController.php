...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('admin.products.create')->with('error', 'تاریخ شروع حراجی صحیح نیست');
        }

        try {
            $sale_date_to = $request->sale_date_to ? convert_jalali_to_gregorian_date($request->sale_date_to) : null;
        } catch (\Exception $e) {
            return redirect()->route('admin.products.create')->with('error', 'تاریخ پایان حراجی صحیح نیست');
        }
    ...
    {
        $product->delete();
        return redirect()->route('admin.products.index')->with('warning', 'محصول با موفقیت حذف شد');
    }
}
