...
@section('content')
    ...
    <form action="{{ route('admin.coupons.update', ['coupon' => $coupon->id]) }}" method="POST" class="row gy-4">
        @csrf
        @method('PUT')
        <div class="col-md-3">
        ...
        <div class="col-md-4">
            ...
            <input data-jdp type="text" name="expired_at" class="form-control"
                value="{{ $coupon->expired_at != null ? convert_gregorian_to_jalali_date($coupon->expired_at) : '' }}">
            <div class="form-text text-danger">
                @error('expired_at')
                    {{ $message }}
                @enderror
